...
SOURCES="ROOT/src/core/*.c ROOT/src/containers/*.c ROOT/src/main.c"
...
if [ -n "$REALLY_QUIET" ]; then
    $CC -c -DPLATFORM_DESKTOP -I$RAYLIB_SRC -I$RAYGUI_SRC -I$ROOT_DIR/src/ $COMPILATION_FLAGS $WARNING_FLAGS $SOURCES > /dev/null 2>&1
    $CC -o $GAME_NAME $ROOT_DIR/$TEMP_DIR/*.o *.o $LINK_FLAGS > /dev/null 2>&1
else
    $CC -c -DPLATFORM_DESKTOP -I$RAYLIB_SRC -I$RAYGUI_SRC -I$ROOT_DIR/src $COMPILATION_FLAGS $WARNING_FLAGS $SOURCES
