...
%CC% -o %SCRIPT_PATH%.bin\%PROJECT_NAME%.html^
 %GAME_SRC% -I. %GAME_INCLUDES% %ENGINE_INCLUDES% %BOX2C_INCLUDES% %SIMDE_INCLUDE% %RAYLIB_INCLUDES% -L. -Lidbfs.js -L%SCRIPT_PATH%.lib^
  %SYSTEM_LIBS% %C_COMPILE_OPTIONS% %SCRIPT_PATH%.lib\engine.a -s ALLOW_MEMORY_GROWTH -s USE_GLFW=3 -s ASSERTIONS=0 -s WASM=1 -s ASYNCIFY -s GL_ENABLE_GET_PROC_ADDRESS=1 -s FORCE_FILESYSTEM=1 --shell-file minishell.html -DPLATFORM_WEB^
