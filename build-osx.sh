...
SRC_DIR="$GAME_NAME/src"
SOURCES="ROOT/$GAME_NAME/src/*.c"

ENGINE_SRC_DIR="engine/src"
ENGINE_SOURCES="ROOT/engine/src/core/*.c ROOT/engine/src/containers/*.c ROOT/engine/src/systems/*.c"

# Set your raylib/src location here (relative path!)
RAYLIB_SRC="external/raylib/src"
RAYGUI_SRC="external/raygui/src"
...
# Directories
ROOT_DIR=$PWD
SOURCES=${SOURCES//ROOT/$ROOT_DIR}
ENGINE_SOURCES=${ENGINE_SOURCES//ROOT/$ROOT_DIR}
RAYLIB_SRC="$ROOT_DIR/$RAYLIB_SRC"
RAYGUI_SRC="$ROOT_DIR/$RAYGUI_SRC"
SRC_DIR="$ROOT_DIR/$SRC_DIR"
ENGINE_SRC_DIR="$ROOT_DIR/$ENGINE_SRC_DIR"
...
if [ ! -d "$TEMP_DIR" ]; then
    ...
    if [ -n "$REALLY_QUIET" ]; then
        ...
        $CC -c $RAYLIB_DEFINES $RAYLIB_INCLUDE_FLAGS -I$RAYGUI_SRC -I$ENGINE_SRC_DIR $COMPILATION_FLAGS $RAYLIB_C_FILES $ENGINE_SOURCES > /dev/null 2>&1
    else
        $CC -c $RAYLIB_DEFINES $RAYLIB_INCLUDE_FLAGS $COMPILATION_FLAGS -x objective-c $RAYLIB_SRC/rglfw.c
        $CC -c $RAYLIB_DEFINES $RAYLIB_INCLUDE_FLAGS -I$RAYGUI_SRC -I$ENGINE_SRC_DIR $COMPILATION_FLAGS $RAYLIB_C_FILES $ENGINE_SOURCES
    fi
